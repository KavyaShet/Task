...
|                                                                                                             |
|  Usage:-                                                                                                    |
|  -> add sender_name receiver_name transfer_message  # adds a block to blockchain with the given data         |
...
const format = (chain: Array<Block>, limit: number) => {
  ...
  newChain.forEach((block, index) => {
    ...
    string += `\n[${index + (isLongList ? 1 : 0)}]\tSender: ${block.data?.from
      }\tReceiver: ${block.data?.to}\tmessage-Transfered: ${block.data?.message}`;
  });
  return string;
};
...
do {
  ...
  switch (command) {
    case "add":
      ...
      const message = commands[3];
      // if (isNaN(message)) {
      //   console.log("<- Please give a correct message");
      //   continue;
      // }
      blockchain.addBlock(commands[1], commands[2], message);
      break;
    case "show":
